...
namespace FacturasApp.Core.Services
{
    public class ExportService
    {
        ...
        {
            var colorHex = estado switch
            {
                EstadoFactura.OK => "#E2EFDA",           // Verde pálido
                EstadoFactura.Revisar => "#FFF2CC",      // Amarillo pálido
                EstadoFactura.Duplicada => "#E6E6FA",    // Lavanda
                EstadoFactura.Error => "#FCE4D6",        // Melocotón pálido
                _ => "#FFFFFF"                            // Blanco
            };

            hoja.Range(fila, 1, fila, numColumnas)
                .Style.Fill.BackgroundColor = XLColor.FromHtml(colorHex);
        }
    }
}